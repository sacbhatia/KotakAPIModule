...
from .settings import exchange_segment as exchange_segment_dict, product as product_dict, order_type as order_type_dict
...
from .settings import help_functions
...
class NeoAPI:
    """
        ...
        Attributes:
            ...
            configuration (NeoUtility): The configuration for the API client.
    ...
    """
        ...
        if not access_token:
            self.configuration = NeoUtility(host=environment)
    ...
    def _get_api(self, api_class):
        """Get cached API instance to avoid repeated instantiation."""
    ...
    def place_order(
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                ...
                exchange_segment = exchange_segment_dict[exchange_segment]
                product = product_dict[product]
                order_type = order_type_dict[order_type]
                place_order = self._get_api(OrderAPI).order_placing(
    ...
    def cancel_order(self, order_id, amo="NO", isVerify=False):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                ...
                cancel_order = self._get_api(OrderAPI).order_cancelling(order_id=order_id,
    ...
    def cancel_cover_order(self, order_id, amo="NO", isVerify=False):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                req_data_validation.cancel_order_validation(order_id)
                cancel_order = self._get_api(OrderAPI).cover_order_cancelling(order_id=order_id,
                                                                                         isVerify=isVerify, amo=amo)
                return cancel_order
            except Exception as e:
                return {'Error': e}
    ...
    def cancel_bracket_order(self, order_id, amo="NO", isVerify=False):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                req_data_validation.cancel_order_validation(order_id)
                cancel_order = self._get_api(OrderAPI).bracket_order_cancelling(order_id=order_id,
                                                                                         isVerify=isVerify, amo=amo)
                return cancel_order
            except Exception as e:
                return {'Error': e}
    ...
    def order_report(self):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                order_list = self._get_api(OrderReportAPI).ordered_books()
                return order_list
            except Exception as e:
                return {'Error': e}
        else:
    ...
    def order_history(self, order_id):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                ...
                history_list = self._get_api(OrderHistoryAPI).ordered_history(order_id=order_id)
                return history_list
            except Exception as e:
                return {'Error': e}
        else:
    ...
    def trade_report(self, order_id=None):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                filtered_trades = self._get_api(TradeReportAPI).trading_report(order_id=order_id)
    ...
    def modify_order(self, order_id, price, order_type, quantity, validity, instrument_token=None,
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            if order_id and instrument_token and exchange_segment and product and trading_symbol:
                ...
                order_type = order_type_dict[order_type]
                try:
                    quick_modify = self._get_api(ModifyOrder). \
                ...
                try:
                    modify_order = self._get_api(ModifyOrder).modification_with_orderid(
    ...
    def positions(self):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                position_list = self._get_api(PositionsAPI).position_init()
    ...
    def holdings(self):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                portfolio_list = self._get_api(PortfolioAPI).portfolio_holdings()
    ...
    def margin_required(self, exchange_segment, price, order_type, product, quantity, instrument_token,
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                req_data_validation.margin_validation(exchange_segment, price, order_type, product, quantity,
                                                                     instrument_token,
                                                                     transaction_type)
                ...
                product = product_dict[product]
                order_type = order_type_dict[order_type]
                margin_required = self._get_api(MarginAPI).margin_init(exchange_segment=exchange_segment,
    ...
    def scrip_master(self, exchange_segment=None):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                scrip_list = self._get_api(ScripMasterAPI).scrip_master_init(
                    exchange_segment=exchange_segment)
                return scrip_list
            except Exception as e:
    ...
    def limits(self, segment="ALL", exchange="ALL", product="ALL"):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                ...
                limits_list = self._get_api(LimitsAPI).limit_init(segment=segment, exchange=exchange,
                                                                                   product=product)
                return limits_list
            except Exception as e:
    ...
    def search_scrip(self, exchange_segment, symbol="", expiry=None, option_type=None, strike_price=None,
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            ...
            try:
                ...
                scrip_list = ScripSearch(self.api_client).scrip_search(exchange_segment=exchange_segment,
    ...
    def subscribe(self, instrument_tokens, isIndex=False, isDepth=False):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            if not self.NeoWebSocket:
                ...
                self.NeoWebSocket = NeoWebSocket(self.configuration.edit_sid,
    ...
    def totp_login(self, mobile_number=None, ucc=None, totp=None):
        ...
        if not mobile_number or not ucc or not totp:
            ...
            return error
        totp_login = self._get_api(TotpAPI).totp_login(mobile_number=mobile_number, ucc=ucc, totp=totp)
        return totp_login
    ...
    def totp_validate(self, mpin=None):
        ...
        totp_validate = self._get_api(TotpAPI).totp_validate(mpin=mpin)
    ...
    def quotes(self, instrument_tokens=None, quote_type=None):
        ...
        if not instrument_tokens:
            ...
            return error
        quotes_response = self._get_api(QuotesAPI).get_quotes(instrument_tokens=instrument_tokens, quote_type=quote_type)
